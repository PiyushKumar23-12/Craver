...
const Home = ({ navigation }) => {
    ...
    const data=groupedBeverages.Beverages.forEach(beverage => {
        console.log(beverage.title);
      });
    const [searchQuery, setSearchQuery] = useState('');

    // Filter the bestsellers based on the search query
    // const filteredBestSellers = BestSellerInfo.filter(item =>
    //     (item.name ? item.name.toLowerCase().includes(searchQuery.toLowerCase()) : false)
    //   );
    //   setSearchQuery(filteredBestSellers)

    const onSearch=(text)=>{
        let tempList=data.filter(item=>{
            item.name ? item.name.toLowerCase().includes(searchQuery.toLowerCase()) : false
        })
        setSearchQuery(tempList)
    }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        ...
        />
        <Octicons name="search"  onPress={text => {
            onSearch(text);setSearchQuery(text)}} size={24} color="#ffffff" />
      </View>
      <ImgSlid />
      <TopCategory />
      <View
      ...
      >
        ...
        ></View>
        <Text style={{ marginVertical: 1, letterSpacing: 6, color: "gray" }}>
          EXPLORE
        </Text>
        <View
          style={{
